...
#if SAVE_ML_FEATURES_CSV
void writeFeatureCsvHeader( std::ostream& os )
{
  os << "VideoName;"
     << "EncoderPreset;"
     << "TargetQP;"
...
void writeFeatureCsvRow( std::ostream& os, const MLFeatureData& data,
                         const std::string& csvVideoName, const std::string& csvEncoderPreset,
                         int csvTargetQp, int csvBitDepth, int csvFrameWidth, int csvFrameHeight )
{
  os << csvVideoName << ";"
     << csvEncoderPreset << ";"
     << csvTargetQp << ";"
...
#if SAVE_ML_FEATURES_CSV
  const std::string defaultFileName = "ml_features_approx_output.csv";
  featFp.open( defaultFileName );
  if( featFp.is_open() )
  {
    writeFeatureCsvHeader( featFp );
  }
  else
  {
    std::cerr << "[Error] MLFeaturesManager: Could not open file " << defaultFileName << std::endl;
  }
#endif
}
...
void MLFeaturesManager::saveFeatures( const MLFeatureData& data )
{
  ...
  writeFeatureCsvRow( featFp, data, videoName, encoderPreset, targetQP, bitDepth, frameWidth, frameHeight );
#endif
}
